#  2021. hvlib.dll python wrapper developed by Arthur Khudyaev (@gerhart_x)
...
class hvlib:
    ...
    def __init__(self, dll_path):
        ...
        self.hvlib.SdkDestroyPartitions.restype = ctypes.c_bool
        self.hvlib.SdkDestroyPartitions.argtypes = []
    ...
    def ReadPhysicalMemoryBlock(self, address, block_size):
        ...
        if not bResult:
            print("ReadPhysicalMemoryBlock error. Address: "+hex(address)+", block_size: "+hex(block_size)+". Zero buffer will be returned")
            return buffer
        # print in hex
    ...
    def ReadVirtualMemoryBlock(self, address, block_size):
        ...
        if not bResult:
            print("ReadVirtualMemoryBlock error. Address: "+hex(address)+", block_size: "+hex(block_size)+". Zero buffer will be returned")
            return 0
    ...
    def cleanup(self):
        self.hvlib.SdkDestroyPartitions()
